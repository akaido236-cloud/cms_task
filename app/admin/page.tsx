...
export default function AdminPage() {
  ...
  function update(path: Array<string | number>, value: ContentValue) {
    setContent((previous) => {
      if (!previous) {
        return previous;
      }

      const next: ContentObject = structuredClone(previous);
  ...
  function addCapability(locale: Locale) {
    setContent((previous) => {
      if (!previous) {
        return previous;
      }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="grid md:grid-cols-2 gap-6">
          ...
          {locales.map((locale) => (
            ...
            >
              {Array.isArray(content.stats?.[locale]) &&
            ...
            >
              {Object.keys(content.about[locale]).map((key) => (
                <TextArea
                  key={key}
                  label={key}
                  value={String(content.about[locale][key] ?? '')}
                  onChange={(v) =>
